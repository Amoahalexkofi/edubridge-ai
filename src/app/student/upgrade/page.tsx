...
import { activeTier, PAYMENTS_ENFORCED } from "@/lib/pricing";
...
export default async function UpgradePage() {
  ...
  const { data: profile } = await supabase
    .from("profiles")
    .select("subscription_tier, subscription_cycle, subscription_expires_at")
    .eq("id", user.id)
    .single();

  const current = activeTier(profile);
